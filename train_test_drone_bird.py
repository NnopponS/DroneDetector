...
def train_and_test_pytorch_cnn(drone_dir, bird_dir, test_drone_dir=None, test_bird_dir=None, epochs=50):
    ...
    # Create model
    model = DroneClassifierPyTorch(model_path='drone_classifier_pytorch.pkl')
    ...
    # Save model
    if model.save('drone_classifier_pytorch.pkl'):
...
def train_and_test_enhanced_ml(drone_dir, bird_dir, test_drone_dir=None, test_bird_dir=None, use_gpu=False):
    ...
    # Create model
    try:
        model = BirdVsDroneClassifier(model_path='bird_v_drone_classifier.pkl', use_gpu=use_gpu)
    except RuntimeError as e:
        print(f"⚠️  GPU not available, using CPU: {e}")
        model = BirdVsDroneClassifier(model_path='bird_v_drone_classifier.pkl', use_gpu=False)
    ...
    # Save model
    if model.save('bird_v_drone_classifier.pkl'):
        print("\n✓ Model saved successfully")
    else:
        print("⚠️  Failed to save model")
...
def main():
    ...
    drone_dir = base_dir / "Drones"
    bird_dir = base_dir / "Birds"
    ...
    if choice in ['1', '3'] and PYTORCH_CNN_AVAILABLE:
        print("  - drone_classifier_pytorch.pkl")
    if choice in ['2', '3'] and ENHANCED_ML_AVAILABLE:
        print("  - bird_v_drone_classifier.pkl")
