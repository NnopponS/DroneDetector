...
class ShapeCodeGenerator:
    ...
    def train(self, drone_dir, bird_dir, output_path='shape_code_model.pkl'):
        """
        ...
        print(f"✓ Shape code model saved to {output_path}")
        return True
    
    def load(self, model_path='shape_code_model.pkl'):
        ...
        try:
            ...
            if self.invert_classification:
                ...
                print(f"✓ Loaded {len(self.drone_shape_codes)} drone codes (from Drones folder) and {len(self.bird_shape_codes)} bird codes (from Birds folder)")
