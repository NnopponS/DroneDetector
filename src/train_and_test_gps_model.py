...
def train_model(samples, model_path=None, epochs=100):
    """เทรนโมเดล GPS"""
    if len(samples) < 10:
    ...
    model_path = model_path or paths.GPS_MODEL_PYTORCH_FILE
    print(f"Model path: {model_path}")
    print(f"{'='*60}\n")
    
    # สร้างโมเดล
    model = GPSModelPyTorch(model_path=model_path)
...
def main():
    ...
    # 2. เทรนโมเดล
    model = train_model(samples, model_path=model_path, epochs=100)
