...
class BirdVsDroneClassifier:
    ...
    def __init__(self, model_path=None, use_gpu=True):
        self.model_path = str(model_path or paths.BIRD_V_DRONE_MODEL)
    ...
    def train(self, drone_dir, bird_dir, use_hog=True, use_hough=True, 
        ...
        if self.use_gpu:
            ...
            self.pytorch_model_hidden_dims = hidden_dims  # Store for saving
    ...
    def save(self, filepath=None):
        ...
        try:
            ...
            if self.use_gpu and self.pytorch_model is not None:
                ...
                # Save hidden_dims if available
                if hasattr(self, 'pytorch_model_hidden_dims'):
                    model_data['pytorch_model_hidden_dims'] = self.pytorch_model_hidden_dims
            else:
    ...
    def load(self, filepath=None):
        ...
        try:
            ...
            if 'pytorch_model_state' in model_data and TORCH_AVAILABLE:
                ...
                # Get hidden_dims from saved data, or infer from state_dict, or use default
                hidden_dims = model_data.get('pytorch_model_hidden_dims', None)
                if hidden_dims is None:
                    # Try to infer hidden_dims from state_dict
                    # Pattern: Linear layers are at indices 0, 4, 8, 12, ... (every 4 layers)
                    # The last linear layer (output) has output size 2
                    state_dict = model_data['pytorch_model_state']
                    hidden_dims = []
                    # Check linear layers at indices 0, 4, 8, 12, 16, ...
                    layer_idx = 0
                    while True:
                        ...
                        if len(weight.shape) == 2:
                            ...
                            # If output size is 2, this is the output layer - stop
                            if out_features == 2:
                                break
                            # Otherwise, it's a hidden layer
                            hidden_dims.append(out_features)
                        # Move to next linear layer (every 4 layers: Linear, BatchNorm, ReLU, Dropout)
                        layer_idx += 4
                    
                    if not hidden_dims:
                        # Fallback to default
                        hidden_dims = [256, 128, 64] if input_dim <= 500 else [512, 256, 128, 64]
                        print(f"⚠ Could not infer hidden_dims from state_dict, using default: {hidden_dims}")
                    else:
                        print(f"✓ Inferred hidden_dims from state_dict: {hidden_dims}")
                
                self.pytorch_model = FeatureClassifierNet(input_dim=input_dim, hidden_dims=hidden_dims, dropout=0.3).to(self.device)
                self.pytorch_model_hidden_dims = hidden_dims  # Store for future saves
                
                try:
                    self.pytorch_model.load_state_dict(model_data['pytorch_model_state'], strict=True)
                except RuntimeError as e:
                    print(f"Load error: {e}")
                    # Try loading with strict=False to handle minor mismatches
                    print("Attempting to load with strict=False...")
                    self.pytorch_model.load_state_dict(model_data['pytorch_model_state'], strict=False)
                    print("⚠ Loaded with strict=False - some weights may not have been loaded")
                
                self.pytorch_model.eval()
                print(f"✓ PyTorch model loaded (device: {self.device.type}, input_dim={input_dim}, hidden_dims={hidden_dims})")
            else:
